 ...
 * {@code int output = random.nextInt(); output = ((output ^ output >>> 17) * 0xD5CB5 | 0)}, which seems to allow the
...
public final class XoRo32RNG implements StatefulRandomness, Serializable { 
    ...
    /**
     * Constructs this XoRo32RNG by calling {@link #setState(long)} on the argument as given; see that method for 
     * the specific details (stateA and stateB are drawn from the lower and upper halves of state, unless state is 0).
     * @param state a long containing both parts of the state; if 0 is given it will be treated as 1
     */
    public XoRo32RNG(final long state) {
        setState(state);
    }
    ...
    public final int next(int bits) {
        ...
        stateB = (s1 << 26 | s1 >>> 6); // c
        return result;
        //return (result ^ result >>> 17) * 0xD5CB5;
    }
    ...
    public final long nextLong() {
        ...
        stateB = (s1 << 26 | s1 >>> 6); // c
        return result;
        //return (result ^ result >>> 17) * 0xD5CB5L;
    }
