 ...
 * equidistributed so it can return all int values but not all longs. Lobster passes at least 4TB of PractRand's
 * statistical tests (tests are ongoing), and does so with only one anomaly and no failures. In
 * statistical testing, xoroshiro128+ always fails some binary matrix rank tests, but that uses a pair of 64-bit states,
 * and when the states are reduced to 32-bits, these small-word versions fail other tests as well. Lobster uses a
 * variant on xoroshiro64** that reduces some issues with that generator and tries to "harden the defenses" on the
 * generator's quality. Lobster does not change xoroshiro's well-tested state transition, but it doesn't base the output
 * on the sum of the two states (like xoroshiro128+), instead using the first state only for output (exactly like
 * xoroshiro64** and similar to xoshiro256**). Any arithmetic it performs is safe for GWT. Starfish adds an extremely
 * small amount of extra code to xoroshiro, running xoroshiro's state transition as normal, using stateA (or s[0] in the
 * original xoroshiro code) multiplied by 31 as the initial result, performing a somewhat-unusual operation on that
 * initial result that isn't common in most RNGs. This last operation is
 * {@code (result << 11) - Integer.rotateLeft(result, 5)}, and this is one of a group of similar scrambling operations
 * with varying effects on quality. It is a random reversible mapping, a one-to-one operation from int to int, but it
 * seems to be a challenge to actually reverse it without creating a lookup table from all int inputs to their outputs.
 * The period is identical to xoroshiro with two 32-bit states, at 0xFFFFFFFFFFFFFFFF or 2 to the 64 minus 1. This
 * generator is a little slower than xoroshiro64+ or Lathe, but has better distribution than either; it can be
 * contrasted with {@link Starfish32RNG} or {@link XoshiroAra32RNG} the former of which is faster than this and the
 * latter of which has a longer period, but both are fragile if users can add integers of their choice.
 * <br>
 ...
 * <br>
 * The name comes from the single sea creature theme I'm using for this family of generators and the hard shell that
 * protects a lobster.
 * <br>
